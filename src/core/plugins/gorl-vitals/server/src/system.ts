...
import { SERVER_EVENT } from '@AthenaPlugins/gorl/shared/events';
...
class InternalFunctions {
    ...
    static handlePing(player: alt.Player) {
        ...
        if (data.isDead) {
            data[VITAL_NAMES.THIRST] = 100;
            data[VITAL_NAMES.HUNGER] = 100;
            Athena.player.emit.meta(player, VITAL_NAMES.HUNGER, data[VITAL_NAMES.HUNGER]);
            Athena.player.emit.meta(player, VITAL_NAMES.THIRST, data[VITAL_NAMES.THIRST]);
            return;
        }

        VitalsSystem.adjustVital(player, VITAL_NAMES.HUNGER, -VITALS_CONFIG.HUNGER_REMOVAL_RATE);
        VitalsSystem.adjustVital(player, VITAL_NAMES.THIRST, -VITALS_CONFIG.THIRST_REMOVAL_RATE);
        VitalsSystem.adjustVital(player, VITAL_NAMES.PEE, -VITALS_CONFIG.PEE_REMOVAL_RATE);
        VitalsSystem.adjustVital(player, VITAL_NAMES.SHIT, -VITALS_CONFIG.SHIT_REMOVAL_RATE);
        VitalsSystem.adjustVital(player, VITAL_NAMES.WASH, -VITALS_CONFIG.WASH_REMOVAL_RATE);
    }
}
...
export class VitalsSystem {
    ...
    static init() {
        ...
        Athena.player.events.on('selected-character', (player) => {
            VitalsSystem.forceUpdateVital(player, VITAL_NAMES.HUNGER);
            VitalsSystem.forceUpdateVital(player, VITAL_NAMES.THIRST);
            VitalsSystem.forceUpdateVital(player, VITAL_NAMES.PEE);
            VitalsSystem.forceUpdateVital(player, VITAL_NAMES.SHIT);
            VitalsSystem.forceUpdateVital(player, VITAL_NAMES.WASH);
        });
    }
    ...
    static adjustVital(player: alt.Player, vitalName: VITAL_NAMES, value: number, isExact = false) {
        ...
        /**
         * player: alt.Player -> target
         * vitalName: VITAL_NAMES
         * value: number/-number -> vital value that added or subtract
         */
        Athena.player.events.trigger(SERVER_EVENT.PLAYER_VITAL_UPDATE, player, vitalName, value);
