...
const STORAGE_BASE_COLLECTION = 'storage_';
const openIdentifiers = [];
const boundIdentifiers: Array<{ id: number; storage: string }> = [];

/**
 * Custom types of Storages , used for the collection name (storage_xxxx)
 */
export enum CustomStorages {
    Houses = 'house',
    ItemContainer = 'itemcontainer',
}

export interface CustomStorageInstance<CustomData = {}> {
    _id?: unknown;
 ...
 */
export async function create(storageType: CustomStorages, items: Array<StoredItem>): Promise<string> {
    if (Overrides.create) {
        return Overrides.create(storageType, items);
    }

    const document = await Database.insertData<CustomStorageInstance>(
        { items, lastUsed: Date.now(), type: storageType },
        STORAGE_BASE_COLLECTION + storageType.toLowerCase(),
        true,
    );
 ...
 */
export async function set(id: string, storageType: CustomStorages, items: Array<StoredItem>): Promise<boolean> {
    if (Overrides.set) {
        return Overrides.set(id, storageType, items);
    }
 ...
 */
export async function get<CustomData = {}>(
    id: string,
    storageType: CustomStorages,
): Promise<Array<StoredItem<CustomData>>> {
    if (Overrides.get) {
        return Overrides.get<CustomData>(id, storageType);
    }

    const document = await Database.fetchData<CustomStorageInstance<CustomData>>(
        '_id',
        id,
        STORAGE_BASE_COLLECTION + storageType.toLowerCase(),
    );
